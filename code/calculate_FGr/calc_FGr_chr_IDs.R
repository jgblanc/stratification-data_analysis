...
cmd_count <- paste("sh code/calculate_FGr/compute_GWAS_count_IDs.sh", gwas_prefix, outfile_count, snps_file, IDs_file, sep = " ")
print(cmd_count)
system(cmd_count)
...
# Compute Gr
cmd_b <- paste("sh code/calculate_FGr/GWAS_score_IDs.sh",
               gwas_prefix,
               paste0(out_prefix, ".xt_temp.glm.linear"),
               paste0(out_prefix,".gxt_tmp"), snps_file, IDs_file, sep = " ")
print(cmd_b)
system(cmd_b)

...
gwasSizeWhite = as.numeric(args[10])
testSize = as.numeric(args[11])
wbs= args[12]
white = args[13]
gwasSizeOther = as.numeric(args[14])
other = args[15]
...
df_other <- inner_join(df,fread(other,  colClasses = 'character')[,1:2])
...
df_GWAS2 <- df_white %>% filter(!IID %in% df_test$IID) %>% sample_n(gwasSizeWhite) %>% select("#FID", "IID")
df_GWAS3 <- df_other %>% filter(!IID %in% df_test$IID) %>% sample_n(gwasSizeOther) %>% select("#FID", "IID")
df_GWAS <- rbind(df_GWAS1, df_GWAS2, df_GWAS3)

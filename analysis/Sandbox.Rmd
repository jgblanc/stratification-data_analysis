...
```{r}
PCs <- fread("~/Downloads/genetic_PC_22009.txt")
FGr <- fread("~/Downloads/FGr-LOCO_sds_22.txt")

df <- inner_join(PCs, FGr)

tmp <- df[,3:42]
tm <- scale(tmp)
mod <- lm(df$FGr ~ tm)
mod2 <- lm(df$FGr ~ ., data=df[,3:42])
cor(df$FGr, fitted(mod2))^2
summary(mod2)

c1 <- as.numeric(unlist(df[,3]))
ct <- cor.test(df$FGr,c1)
ct
ct$conf.int[1:2]   

...
ggplot(df, aes(y = phenotype, x = pc, fill = q)) + geom_tile() + facet_wrap(~covar) + scale_fill_gradient(low="white", high="blue") + geom_text(aes(label=sig), color="black", size=5) 
```

```{r}
ggplot(df_covar, aes(y = phenotype, x = pc, fill = diff)) + geom_tile()+ scale_fill_gradient2(low="red", mid = "white", high="blue")
```

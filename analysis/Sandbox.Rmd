...
```{r}
df <- fread("../plots/hgdp/EUR/WBS/results_LR.txt")
df$pc <- as.character(df$pc)
df <- df %>% mutate(sig = case_when(p <= 0.05 ~ "*", p > 0.05 ~ ""))
df <- df %>% separate("phenotype", c("phenotype", "tmp1", "tmp2"))
...
pl2 <- ggplot(df, aes(y = phenotype, x = contrast, fill = q_NoSign)) + geom_tile() + facet_wrap(~covar, labeller = hospital_labeller) + scale_fill_gradient2(low = "brown4", high = "royalblue4", name = TeX("$\\hat{q}$")) + geom_text(aes(label=sigNoSign), color="black", size=7, fontface="bold")  + ylab("") + xlab("Test Vector") + theme_bw(base_size = 14) + theme(axis.text = element_text(color = "black"), plot.title = element_text(hjust = 0.5)) + ggtitle("No Effect Sizes")
...
ggsave("~/Desktop/HGDPLatLongWBS.png",pl, width = 15, height = 8)
...
#ggsave("~/Desktop/HGDP_Regions.png",pl, width = 15, height = 8)

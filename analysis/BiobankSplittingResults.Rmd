...
```{r}
df <- fread("../plots/AA/resultsEur3.txt")
...
df$pc <- as.factor(df$pc)
df$testSize <- as.factor(df$testSize)
df_plot <- df %>% filter(rep == "tp3s")
EA <- df_plot %>% filter(phenotype == "Educational_Attainment_6138")
Ht <- df_plot %>% filter(phenotype == "StandingHeight_50")

ggplot(data = EA, aes(x = pc, y = exp, fill = q)) + geom_tile() + facet_grid(vars(covar), vars(contrast)) + scale_fill_gradient2(low = "brown4", high = "royalblue4", name = TeX("$\\hat{q}$")) + geom_text(aes(label=sig), color="black", size=7, fontface="bold") + theme_bw(base_size = 14) + ylab(" ")

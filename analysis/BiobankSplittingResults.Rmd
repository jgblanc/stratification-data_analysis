...
ggplot(data = df, aes(x = pc, y = testSize, fill = q)) + geom_tile() + facet_grid(vars(covar), vars(contrast)) + scale_fill_gradient2(low = "brown4", high = "royalblue4", name = TeX("$\\hat{q}$")) + geom_text(aes(label=sig), color="black", size=7, fontface="bold") + theme_bw(base_size = 14)

```

```{r}
df <- fread("../plots/AA/tp1_100000/gp300000/sharp_PCs_FGr.txt")

...
```{r}
df <- fread("../plots/SS/eur50000/wbs300000/results_PCs.txt")
...
ggplot(data = df, aes( x = contrast, y = pc, fill = q)) + geom_tile() + facet_wrap(~covar) + scale_fill_gradient2(low = "brown4", high = "royalblue4", name = TeX("$\\hat{q}$")) + geom_text(aes(label=sig), color="black", size=7, fontface="bold") + theme_bw(base_size = 14) 
#ggsave("~/Desktop/North.png", pl_North)
```
```

...
ggplot(data = res_smooth, aes(x = phenotype, y  = mean_q, color = pc )) + geom_point() + facet_wrap(~covar)

res_sharp <- res %>% filter(contrast == "sharp")
ggplot(data = res_sharp, aes(x = phenotype, y  = mean_q, color = pc )) + geom_point() + facet_wrap(~covar)
```


```{r}
Var <-  fread("../plots/Sims/FGrError.txt")
dfVar <- Var %>% group_by(contrast) %>% summarise(meanVar = mean(varE), lc = ) 

...
```{r}
df1 <- fread("../plots/AA/results.txt")
df1$exp <- paste0("Test ", df1$testSize, " GWAS ", df1$gwasSize)
...
ggplot(data = df, aes(x = pc, y = exp, fill = q)) + geom_tile() + facet_grid(vars(covar), vars(contrast)) + scale_fill_gradient2(low = "brown4", high = "royalblue4", name = TeX("$\\hat{q}$")) + geom_text(aes(label=sig), color="black", size=7, fontface="bold") + theme_bw(base_size = 14) + ylab(" ")

```

```{r}
df <- fread("../plots/AA/tp1_200000/gp200000/north_PCs_FGr.txt")

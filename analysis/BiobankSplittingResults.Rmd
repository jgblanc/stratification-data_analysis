...
ggplot(data = df, aes(x = pc, y = contrast, fill = q)) + geom_tile() + facet_wrap(~covar) + scale_fill_gradient2(low = "brown4", high = "royalblue4", name = TeX("$\\hat{q}$")) + geom_text(aes(label=sig), color="black", size=7, fontface="bold") + theme_bw(base_size = 14)
```
...
```{r}
varG <- (21/22) * sum((gammas -  mean(gammas))^2)
sqrt(varG)
```
